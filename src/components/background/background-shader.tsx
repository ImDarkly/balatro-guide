...
export default function BackgroundShader({
  ...
  useEffect(() => {
    ...
    function update(time: number) {
      if (!pauseRef.current) {
        ...
        const t = (time - startTimeRef.current) * 0.001 + timeOffsetRef.current;
  ...
  useEffect(() => {
    if (programRef.current && rendererRef.current && meshRef.current) {
      if (pauseRef.current) {
        programRef.current.uniforms.iTime.value = lastTimeRef.current;
        rendererRef.current.render({ scene: meshRef.current });
      } else {
        if (startTimeRef.current !== 0) {
          timeOffsetRef.current = lastTimeRef.current;
